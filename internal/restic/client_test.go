...
func TestResticHelper(t *testing.T) {
	...
	if os.Getenv("RESTIC_HELPER_LOCKS") == "1" {
		...
		case strings.Contains(joined, "list locks"):
			ids := os.Getenv("RESTIC_HELPER_LOCK_IDS")
			if ids == "" {
				ids = "abc123"
			}
			_, _ = io.WriteString(os.Stdout, ids+"\n")
		case strings.Contains(joined, "cat lock"):
			lockJSON := os.Getenv("RESTIC_HELPER_LOCK_JSON")
			if encoded := os.Getenv("RESTIC_HELPER_LOCK_JSONS"); encoded != "" {
				locks := make(map[string]string)
				_ = json.Unmarshal([]byte(encoded), &locks)
				lockJSON = locks[arguments[len(arguments)-1]]
			}
...
func TestRecoverRepositoryLocksChecksEveryLock(t *testing.T) {
	previous := isProcessActive
	t.Cleanup(func() { isProcessActive = previous })
	isProcessActive = func(int) (bool, error) { return false, nil }
	now := time.Now().UTC()
	...
	fresh, err := json.Marshal(repositoryLock{Time: now, PID: 1001, Hostname: host})
	if err != nil {
		t.Fatal(err)
	}
	stale, err := json.Marshal(repositoryLock{Time: now.Add(-2 * time.Hour), PID: 1002, Hostname: host})
	if err != nil {
		t.Fatal(err)
	}
	locks, err := json.Marshal(map[string]string{"aaa111": string(fresh), "bbb222": string(stale)})
	if err != nil {
		t.Fatal(err)
	}
	logPath := filepath.Join(t.TempDir(), "restic.json")
	client := &Client{executable: os.Args[0], prefixArguments: []string{"-test.run=TestResticHelper", "--"}, stdout: io.Discard, stderr: io.Discard}
	config := Config{Repository: "local:repository", PasswordValue: "secret", Environment: map[string]string{
		"GO_WANT_RESTIC_HELPER": "1", "RESTIC_HELPER_LOCKS": "1", "RESTIC_HELPER_LOCK_IDS": "aaa111\nbbb222",
		"RESTIC_HELPER_LOCK_JSONS": string(locks), "RESTIC_HELPER_LOG": logPath,
	}}
	if err := client.RecoverRepositoryLocks(context.Background(), config, time.Hour, false, now); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(strings.Join(result.Arguments, " "), "unlock") {
		t.Fatal("later stale lock was not unlocked")
	}
}
