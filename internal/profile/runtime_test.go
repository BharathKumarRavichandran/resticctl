...
func TestValidateRuntimeNormalizesLock(t *testing.T) {
	...
	if err := validateRuntime(&value, base); err != nil {
		t.Fatal(err)
	}
	if value.Runtime.Lock.Path != filepath.Join(base, "locks", "job.lock") {
		t.Fatalf("lock path = %q", value.Runtime.Lock.Path)
	}
}
...
func TestValidateRuntimeRejectsUnsafePolicies(t *testing.T) {
	...
	for _, policy := range tests {
		...
		if err := validateRuntime(&value, t.TempDir()); err == nil {
			t.Fatalf("accepted %#v", policy)
		}
	}
	if runtime.GOOS != "linux" {
		value := Profile{Runtime: Runtime{IOPriority: &IOPriority{Class: "idle"}}}
		if err := validateRuntime(&value, t.TempDir()); err == nil || !strings.Contains(err.Error(), "Linux") {
			t.Fatalf("ionice error = %v", err)
		}
	}
}
